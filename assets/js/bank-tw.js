...
//            debugger;
            ...
            console.log('*************************')
                ...
                var balance_new = res.total[0].value.split('+');
                $('#div_total').html(balance_new[1]+" THB");
								$('#d_now').html(d_now);
								console.log(balance_new)
								console.log('*************************')
								func_add_detailbank($('#i_bank').val(),d_now,balance_new[1]);
                console.log('*************************')
                ...
                $.each(res.transaction, function (j, item) {
                        ...
                        console.log(item.datetime + " | " + item.info + " | " + item.out + " | " + item.in + " | " + item.transactionCode + " | " + item.channel);
console.log('*************************')

                    append_msg +="<tr><td>"+i_rows+"</td><td>"+item.datetime+"</td><td>"+item.info+"</td><td>"+item.out+"</td><td>"+item.in+"</td> <td>"+item.channel+"</td></tr>";
                    i_rows++;
                    });

				var append_thead_trans = "<tr> <th class='text-center'>#</th> <th>วันที่</th> <th>รายการ</th> <th>ถอน</th> <th>ฝาก</th>   <th>ช่องทาง</th> </tr>";
				$('#thead_trans').append(append_thead_trans);
				if(i_rows > 1){
					$('#tbody_trans').append(append_msg);
				}else{
					append_msg = "<tr><td colspan='6' align='center'> no data available in table </td></tr>";
					$('#tbody_trans').append(append_msg);
				}
